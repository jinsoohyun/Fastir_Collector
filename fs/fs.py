...
class _FS(object):
	...
	def _csv_hosts_file(self, hosts_file):
		with open(self.output_dir + '\\' + self.computer_name + 'hostsFile' + self.rand_ext, 'wb') as output:
			csv_writer = get_csv_writer(output)
			write_to_csv(("COMPUTER_NAME", "TYPE", "IP", "URL"), csv_writer)
			
			for hosts_ip, hosts_url in hosts_file:
				write_to_csv([self.computer_name, 'Hosts_File', hosts_ip, hosts_url], csv_writer)
				
		record_sha256_logs(self.output_dir + '\\' + self.computer_name + 'hosts_file' + self.rand_ext,
						   self.output_dir + '\\' + self.computer_name + '_sha256.log')

	def _json_hosts_history(self, hosts_file):
		if self.destination == 'local':
			with open(os.path.join(self.output_dir, '%s_hosts_file.json') % self.computer_name, 'wb') as output:
				json_writer = get_json_writer(output)
				header = ["COMPUTER_NAME", "TYPE", "IP", "URL"]
				for time, url, title, user, profile in hosts_file:
					write_to_json(header, [self.computer_name, 'Hosts_File',
										   hosts_ip, hosts_url], json_writer)
